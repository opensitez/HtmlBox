...
fn measure_content_height(
    engine: &LayoutEngine, child: &mut WebCore,
    content_w: f32, content_x: f32, content_y: f32,
    font_px: f32, root_font_px: f32,
) -> f32 {
    let saved = if child.style.height.is_auto() { None } else {
...
pub fn layout_flex(
    ...
    if let Some(kind) = node.style.width.intrinsic().filter(|_| rbox.content_width.is_none()) {
        let avail = content_w;
    ...
    // on one line.
    let mut definite_main: Option<f32> = if is_row { Some(content_w) } else { rbox.content_height };
    ...
    let (col_min_main, col_max_main) = if is_row { (0.0, f32::MAX) } else {
        ...
        let mn = (node.style.min_height.resolve_vp(font_px, 0.0, root_font_px, engine.viewport_w, engine.viewport_h) - bb).max(0.0);
        let mx = if node.style.max_height.is_none() || node.style.max_height.is_auto() { f32::MAX } else {
            (node.style.max_height.resolve_vp(font_px, 0.0, root_font_px, engine.viewport_w, engine.viewport_h) - bb).max(0.0)
        };
        (mn, mx)
    };
    ...
    let column_gap = node.style.column_gap.resolve_vp(font_px, content_w, root_font_px, engine.viewport_w, engine.viewport_h);
    let row_gap    = node.style.row_gap.resolve_vp(font_px, content_h_basis, root_font_px, engine.viewport_w, engine.viewport_h);
    ...
    for path in &child_paths {
        ...
        let saved_margins = if auto_margins.iter().any(|&a| a) {
            ...
            let saved = [st.margin_top.clone(), st.margin_right.clone(),
                         st.margin_bottom.clone(), st.margin_left.clone()];
            if st.margin_top.is_auto()    { st.margin_top    = CssLength::Zero; }
            if st.margin_right.is_auto()  { st.margin_right  = CssLength::Zero; }
            if st.margin_bottom.is_auto() { st.margin_bottom = CssLength::Zero; }
            if st.margin_left.is_auto()   { st.margin_left   = CssLength::Zero; }
            Some(saved)
        } else { None };
        ...
        let transferred_main: Option<f32> = match child.style.aspect_ratio {
            Some(ratio) if ratio > 0.0 => {
                if is_row {
                    if child.style.height.is_auto() { None } else {
                        let raw = child.style.height.resolve_vp(child_font, rbox.content_height.unwrap_or(0.0), root_font_px, engine.viewport_w, engine.viewport_h);
                ...
                } else if child.style.width.is_auto() { None } else {
        ...
        let basis_is_content = matches!(child.style.flex_basis, CssLength::Content)
            || basis_is_percent_auto;
        ...
        let intrinsic_basis = child.style.flex_basis.intrinsic().or_else(|| {
            ...
            if child.style.flex_basis.is_auto() {
                if is_row { child.style.width.intrinsic() } else { child.style.height.intrinsic() }
            } else { None }
        });
        ...
        let basis_main: f32 = if let Some(kind) = intrinsic_basis {
            if is_row {
                match kind {
                    CssLength::MinContent => engine.min_content_width_of_content(child, font_px, root_font_px),
                    CssLength::MaxContent => engine.max_content_width_of_content(child, font_px, root_font_px),
                ...
                measure_content_height(engine, child, content_w, content_x, content_y, font_px, root_font_px)
            }
        ...
        let specified_sugg: Option<f32> = {
            let len = if is_row { &child.style.width } else { &child.style.height };
            // A percentage against an indefinite container main size is not
            // definite, so it is no suggestion at all.
            let definite = !len.is_auto() && len.intrinsic().is_none()
                && !(matches!(len, CssLength::Percent(_)) && !is_row && rbox.content_height.is_none());
            if definite {
                let basis = if is_row { content_w } else { rbox.content_height.unwrap_or(0.0) };
                let v = len.resolve_vp(child_font, basis, root_font_px, engine.viewport_w, engine.viewport_h);
        ...
        let auto_min_main = move |content_sugg: f32| -> f32 {
            ...
            if let Some(sp) = specified_sugg { m = m.min(sp); }
        ...
        let min_main: f32 = if is_row {
            ...
            if !child.style.min_height.is_auto() {
                ...
                auto_min_main(measure_content_height(engine, child, content_w, content_x, content_y, font_px, root_font_px))
            }
        };
    ...
    let mut lines: Vec<FlexLine> = Vec::new();
    {
        ...
        while i < items.len() {
            ...
            while i < items.len() {
                ...
                if can_wrap && count > 0
                    && total_with_gap > definite_main.unwrap_or(f32::INFINITY) {
                    break;
                }
    ...
    for line in &lines {
        ...
        for idx in range.clone() {
            ...
            let past = if growing { it.base_main > it.hyp } else { it.base_main < it.hyp };
        ...
        let initial_free = {
            ...
            for (j, idx) in range.clone().enumerate() {
                used += items[idx].outer_extra
                    + if frozen[j] { items[idx].main_used } else { items[idx].base_main };
            }
        ...
        while frozen.iter().any(|f| !f) {
            ...
            let total: f32 = range.clone().enumerate()
                .filter(|(j, _)| !frozen[*j])
                .map(|(_, idx)| if growing {
                    items[idx].flex_grow
                } else {
            ...
            let factor_sum: f32 = range.clone().enumerate()
                .filter(|(j, _)| !frozen[*j])
                .map(|(_, idx)| if growing { items[idx].flex_grow } else { items[idx].flex_shrink })
                .sum();
            if factor_sum < 1.0 {
                let scaled = initial_free * factor_sum;
                if scaled.abs() < free.abs() { free = scaled; }
            }
            ...
            for (j, idx) in range.clone().enumerate() {
                ...
                let clamped = unclamped.max(items[idx].min_main).min(items[idx].max_main).max(0.0);
                items[idx].main_used = clamped;
                violation += clamped - unclamped;
                if clamped > unclamped { min_violators.push(j); }
                if clamped < unclamped { max_violators.push(j); }
            }

            if violation > 0.0 {
                for j in min_violators { frozen[j] = true; }
            } else if violation < 0.0 {
                for j in max_violators { frozen[j] = true; }
            } else {
                for f in frozen.iter_mut() { *f = true; }
            }
        }
    }
    ...
    for item in &mut items {
        ...
        engine.layout_box(child, &item_constraints(rbox.content_height, 
        ...
        let wants_baseline = match child.style.align_self {
            ...
            AlignSelf::Auto     => matches!(parent_align, AlignItems::Baseline | AlignItems::LastBaseline),
            _ => false,
        };
        item.baseline_off = if is_row && wants_baseline { first_baseline_offset(child) } else { None };
    }
    ...
    for line in &mut lines {
        ...
        if above + below > line.cross_size { line.cross_size = above + below; }
    }
    ...
    if !can_wrap && lines.len() == 1 {
        let definite_cross = if is_row { rbox.content_height } else { Some(content_w) };
    ...
    for li in 0..lines.len() {
        ...
        let has_main_auto = items[lines[line_idx].start..lines[line_idx].start + lines[line_idx].count]
            .iter().any(|item| item.auto_margins[0] || item.auto_margins[1]);
        ...
        let (main_start, main_extra_gap) = if has_main_auto {
            ...
            for j in 0..lines[line_idx].count {
                ...
                if am[0] { auto_count += 1; }
                if am[1] { auto_count += 1; }
            }
            ...
            for ii in 0..lines[line_idx].count {
                ...
                if items[idx].auto_margins[0] { auto_pos += auto_margin_size; }
                items[idx].main_pos = auto_pos;
                auto_pos += items[idx].main_used + items[idx].outer_extra;
                if items[idx].auto_margins[1] { auto_pos += auto_margin_size; }
                auto_pos += gap_main;
            }
            (0.0, gap_main) // signal that main_pos was already set
        } else {
            justify_spacing(node.style.justify_content, free_main, lines[line_idx].count, gap_main, is_reversed,
        ...
        for ii in 0..lines[line_idx].count {
            ...
            let (line_baseline, line_baseline_group_depth) = {
                ...
                if any { (Some(above), above + below) } else { (None, 0.0) }
            };
            let eff_align = effective_align_self(child_ref(node, &items[item_idx].path), node.style.align_items, wrap_reverse);
            ...
            let cross_pos = if let (Some(lb), Some(ib)) = (line_baseline, item_baseline) {
                ...
                if is_row {
                    ...
                    if cross_is_auto
                        && target_h > 0.0 && (target_h - child.layout.content_rect.h).abs() > 0.5 {
                        child.layout.layout_dirty = true;
                        ...
                        engine.layout_box(child, &item_constraints(rbox.content_height, 
                            item_containing, content_x, content_y, font_px, root_font_px,
                            Some(items[item_idx].main_used), Some(target_h)));
                    ...
                    if cross_is_auto
                        && stretch_w > 0.0 && (stretch_w - child.layout.content_rect.w).abs() > 0.5 {
                        child.layout.layout_dirty = true;
                        ...
                        engine.layout_box(child, &item_constraints(rbox.content_height, 
                            ...
                            Some(stretch_w), Some(items[item_idx].main_used)));
                ...
                if !is_row {
                    ...
                    if child_width_is_auto {
                        ...
                        if intrinsic_w < items[item_idx].cross_size - 0.5 {
                            ...
                            engine.layout_box(child, &item_constraints(
                                rbox.content_height, intrinsic_w, content_x, content_y,
                                font_px, root_font_px, None, Some(items[item_idx].main_used)));
                            items[item_idx].cross_size = child.layout.margin_rect.w;
                        }
                    }
                }
                ...
                let safe = {
                    ...
                    let style = if matches!(ci.style.align_self, AlignSelf::Auto) { &node.style } else { &ci.style };
...
fn justify_spacing(
    ...
    let jc = if free < 0.0 {
        ...
        if safe { JustifyContent::FlexStart } else {
            match jc {
...
fn item_constraints(
    ...
    available_width: f32,
    x: f32, y: f32,
    parent_font_px: f32, root_font_px: f32,
    forced_width: Option<f32>, forced_height: Option<f32>,
) -> Constraints {
    let mut c = Constraints::with_forced(available_width, x, y, parent_font_px, root_font_px,
