...
fn walk_all(d: &mut Document, t: u32) -> Vec<String> {
    ...
    while let Some(n) = d.tw_next_node(t) { out.push(nm(d, n)); }
    out
}

fn iter_all(d: &mut Document, t: u32) -> Vec<String> {
    let mut out = Vec::new();
    while let Some(n) = d.ni_next_node(t) { out.push(nm(d, n)); }
    out
}
...
fn what_to_show_selects_which_node_kinds_the_walk_returns() {
    ...
    assert_eq!(texts[0], "root", "currentNode starts at the root whatever whatToShow says");
...
fn the_filter_is_never_called_for_a_node_what_to_show_excluded() {
    ...
    assert_eq!(kinds.len(), 5, "one call per ELEMENT below the root, got {kinds:?}");
    assert!(kinds.iter().all(|k| *k == 1), "only elements reached the filter: {kinds:?}");
}
...
fn the_walker_never_returns_its_root_and_the_iterator_returns_it_first() {
    ...
    let root = el(&d, "root");
    let t = d.create_tree_walker(root, SHOW_ELEMENT, None);
    assert_eq!({ let n = d.tw_next_node(t).unwrap(); nm(&d, n) }, "p1", "the walker starts BELOW the root");

    let it = d.create_node_iterator(root, SHOW_ELEMENT, None);
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "root", "the iterator returns it");
}
...
fn the_root_is_never_filtered() {
    ...
    d.tw_next_node(t);
    assert_eq!(*seen.lock().unwrap(), ["p1"], "the walk moves off the root before asking");
}
...
fn reject_prunes_a_subtree_for_the_walker_and_skip_does_not() {
    ...
    let t = d.create_tree_walker(root, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
        if n == p1 { FILTER_SKIP } else { FILTER_ACCEPT }
    })));
    assert_eq!(walk_all(&mut d, t), ["root", "b1", "p2", "s1", "i1"]);

    let t = d.create_tree_walker(root, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
        if n == p1 { FILTER_REJECT } else { FILTER_ACCEPT }
    })));
    assert_eq!(walk_all(&mut d, t), ["root", "p2", "s1", "i1"], "b1 went with p1");
}
...
fn reject_is_exactly_skip_for_an_iterator() {
    ...
    let root = el(&d, "root");
    let p1 = el(&d, "p1");
    let it = d.create_node_iterator(root, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
        if n == p1 { FILTER_REJECT } else { FILTER_ACCEPT }
    })));
...
fn previous_node_from_the_end_differs_by_one_between_the_two_objects() {
    ...
    while let Some(n) = d.tw_previous_node(t) { back.push(nm(&d, n)); }
    assert_eq!(back, ["s1", "p2", "b1", "p1", "root"]);

    let it = d.create_node_iterator(root, SHOW_ELEMENT, None);
    while d.ni_next_node(it).is_some() {}
    let mut back = Vec::new();
    while let Some(n) = d.ni_previous_node(it) { back.push(nm(&d, n)); }
    assert_eq!(back, ["i1", "s1", "p2", "b1", "p1", "root"], "i1 comes back first");
}
...
fn first_child_next_sibling_and_parent_node_move_the_current_node() {
    ...
    let root = el(&d, "root");
    let t = d.create_tree_walker(root, SHOW_ELEMENT, None);
    assert_eq!(d.tw_first_child(t).map(|n| nm(&d, n)).as_deref(), Some("p1"));
    assert_eq!(d.tw_next_sibling(t).map(|n| nm(&d, n)).as_deref(), Some("p2"));
    assert_eq!(d.tw_parent_node(t).map(|n| nm(&d, n)).as_deref(), Some("root"));
...
fn a_navigation_that_finds_nothing_leaves_the_current_node_where_it_was() {
    ...
    let t = d.create_tree_walker(root, SHOW_ELEMENT, None);
    assert_eq!(d.tw_parent_node(t), None, "the root has no parent inside the walk");
...
fn first_child_descends_through_a_skip_and_steps_over_a_reject() {
    ...
    assert_eq!(d.tw_first_child(t).map(|n| nm(&d, n)).as_deref(), Some("b1"));

    let t = d.create_tree_walker(root, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
        if n == p1 { FILTER_REJECT } else { FILTER_ACCEPT }
    })));
    assert_eq!(d.tw_first_child(t).map(|n| nm(&d, n)).as_deref(), Some("p2"));
}
...
fn a_root_with_no_matching_descendants_yields_nothing() {
    ...
    assert_eq!(d.tw_first_child(t).map(|n| d.node_type(n)), Some(3), "its text node");
}
...
fn first_child_stops_at_the_node_the_walk_started_from() {
    ...
    for last in [false, true] {
        let t = d.create_tree_walker(r, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
            if n == b1 { FILTER_REJECT } else { FILTER_ACCEPT }
        })));
        d.set_current_node(t, p1);
        let got = if last { d.tw_last_child(t) } else { d.tw_first_child(t) };
...
fn first_child_through_a_skip_still_stops_at_the_start() {
    ...
    let i1 = el(&d, "i1");
    let t = d.create_tree_walker(r, SHOW_ELEMENT, Some(Box::new(move |_: &Document, n: u32| {
        if n == b1 || n == i1 { FILTER_SKIP } else { FILTER_ACCEPT }
    })));
    d.set_current_node(t, p1);
...
fn the_iterator_pointer_starts_before_its_reference_and_flips_on_the_first_step() {
    ...
    d.ni_next_node(it);
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "root", "still the root");
    assert_eq!(d.pointer_before_reference_node(it), Some(false), "the pointer moved past it");
}
...
fn detach_does_nothing_at_all() {
    ...
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "root", "still works afterwards");
}

// ─── removal: the pre-removing steps ────────────────────────────────────────

const DEEP: &str = "<div id=r><p id=p1><b id=b1><i id=i1></i></b></p><p id=p2></p><p id=p3></p></div>";

fn deep() -> Document { parse_html(DEEP) }
...
fn deep2() -> Document { parse_html(DEEP2) }
...
fn removing_an_ancestor_of_the_reference_moves_it_back_past_the_subtree() {
    ...
    d.ni_next_node(it); d.ni_next_node(it); d.ni_next_node(it); // r, p1, b1
    ...
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "p2");
}
...
fn removing_a_deep_reference_walks_all_the_way_out_of_the_removed_subtree() {
    ...
    let r = el(&d, "r");
    let it = d.create_node_iterator(r, SHOW_ELEMENT, None);
    for _ in 0..4 { d.ni_next_node(it); } // r, p1, b1, i1
...
fn removing_a_node_that_is_not_an_ancestor_leaves_the_reference_alone() {
    ...
    let r = el(&d, "r");
    let it = d.create_node_iterator(r, SHOW_ELEMENT, None);
    for _ in 0..5 { d.ni_next_node(it); } // r, p1, b1, i1, p2
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "p2");
    let p1 = el(&d, "p1");
    d.remove_child(p1);
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "p2", "untouched");
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "p3");

    let mut d = deep();
    let r = el(&d, "r");
    let it = d.create_node_iterator(r, SHOW_ELEMENT, None);
    d.ni_next_node(it); d.ni_next_node(it); // r, p1
    let p3 = el(&d, "p3");
    d.remove_child(p3);
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "p1");
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "b1");
}
...
fn with_the_pointer_before_the_reference_the_removal_moves_it_forward_instead() {
    ...
    let r = el(&d, "r");
    let it = d.create_node_iterator(r, SHOW_ELEMENT, None);
    d.ni_next_node(it); d.ni_next_node(it); d.ni_previous_node(it);
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "p1");
    assert_eq!(d.pointer_before_reference_node(it), Some(true));
    let p1 = el(&d, "p1");
    d.remove_child(p1);
...
fn the_reference_lands_on_the_previous_siblings_deepest_descendant() {
    ...
    let r = el(&d, "r");
    let it = d.create_node_iterator(r, SHOW_ELEMENT, None);
    for _ in 0..7 { d.ni_next_node(it); } // r, p1, b1, i1, p2, b2, i2
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "i2");
    let p2 = el(&d, "p2");
    d.remove_child(p2);
    assert_eq!(nm(&d, d.reference_node(it).unwrap()), "i1", "not the parent r");
    assert_eq!(d.pointer_before_reference_node(it), Some(false));
    assert_eq!({ let n = d.ni_next_node(it).unwrap(); nm(&d, n) }, "p3");
}
...
fn a_tree_walker_is_not_told_about_removals() {
    ...
    assert_eq!(d.current_node(t), Some(p1));
    d.remove_child(p1);
    assert_eq!(d.current_node(t), Some(p1), "still pointing at the detached node");
}
...
fn the_handle_reports_what_it_was_created_with() {
    ...
    let t2 = d.create_tree_walker(root, SHOW_ALL, Some(Box::new(|_: &Document, _| FILTER_ACCEPT)));
...
fn a_cloned_document_carries_no_traversals() {
    ...
    let t = d.create_tree_walker(root, SHOW_ELEMENT, None);
    let clone = d.clone();
    assert_eq!(clone.traversal_root(t), None, "the handle names nothing in the clone");
    assert_eq!(d.traversal_root(t), Some(root), "and still works in the original");
}
...
fn a_parsed_comment_is_a_comment_node_not_an_element() {
    ...
    assert_eq!(d.node_name(comment), "#comment", "not uppercased — it is not an element");
