...
fn find_by_tag<'a>(node: &'a WebCore, tag: &str) -> Option<&'a WebCore> {
    ...
    if let Some(ref sr) = node.shadow_root {
        for child in &sr.children { if let Some(n) = find_by_tag(child, tag) { return Some(n); } }
    }
    None
}
...
fn find_by_id<'a>(node: &'a WebCore, id: &str) -> Option<&'a WebCore> {
    ...
    if let Some(ref sr) = node.shadow_root {
        for child in &sr.children { if let Some(n) = find_by_id(child, id) { return Some(n); } }
    }
    None
}
...
fn declarative_shadow_dom_creates_shadow_root() {
    ...
    assert!(host.children.iter().any(|c| c.tag == "span"), "light DOM span should remain");
}
...
fn document_styles_do_not_leak_into_shadow() {
    ...
    assert_ne!(shadow_div.style.color, Color::rgb(0, 128, 0),
...
fn shadow_styles_do_not_leak_out() {
    ...
    assert_ne!(outside.style.color, Color::rgb(255, 0, 0),
...
fn shadow_scoped_styles_apply_inside() {
    ...
    assert!((fs - 24.0).abs() < 0.1,
        "shadow scoped style should apply: expected 24px, got {}", fs);
}
...
fn inherited_properties_cross_shadow_boundary() {
    ...
    assert!((fs - 20.0).abs() < 0.1,
        "font-size should inherit across shadow boundary: expected 20px, got {}", fs);
}
...
fn attach_shadow_programmatic() {
    ...
    host.attach_shadow(ShadowMode::Open, r#"
