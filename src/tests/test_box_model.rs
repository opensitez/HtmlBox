...
fn boxmodel_margin_auto_center() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && (b.content_rect.w - 200.0).abs() < 5.0);
    assert!(b.is_some(), "div with width=200 not found");
    let x = b.unwrap().content_rect.x;
...
fn boxmodel_margin_auto_left_only() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && (b.content_rect.w - 200.0).abs() < 5.0);
    assert!(b.is_some());
    assert!(b.unwrap().content_rect.x > 500.0);
}
...
fn boxmodel_margin_auto_does_not_affect_defaults() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && (b.content_rect.w - 200.0).abs() < 5.0);
    assert!(b.is_some());
    assert!(b.unwrap().content_rect.x < 50.0);
}
...
fn boxmodel_padding_affects_layout() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && (b.content_rect.w - 200.0).abs() < 5.0);
    assert!(b.is_some());
    let pw = b.unwrap().padding_rect.w;
...
fn boxmodel_content_box_default() {
    ...
    let bx = b.unwrap();
    assert!((bx.content_rect.w - 200.0).abs() < 5.0, "content width should be 200");
    // borderRect = 200 (content) + 40 (padding) + 10 (border) = 250
    assert!(bx.border_rect.w >= 245.0 && bx.border_rect.w <= 255.0,
        "expected border_rect.w ~250, got {}", bx.border_rect.w);
}
...
fn boxmodel_border_box_sizing() {
    ...
    assert!(bx.border_rect.w >= 195.0 && bx.border_rect.w <= 205.0,
        "expected border_rect.w ~200, got {}", bx.border_rect.w);
    // contentWidth = 200 - 40 (padding) - 10 (border) = 150
    assert!(bx.content_rect.w >= 145.0 && bx.content_rect.w <= 155.0,
        "expected content_rect.w ~150, got {}", bx.content_rect.w);
}
...
fn boxmodel_explicit_width() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && (b.content_rect.w - 300.0).abs() < 5.0);
    assert!(b.is_some());
}
...
fn boxmodel_auto_width_fills_container() {
    ...
    let b = find_box(&doc.root, &|b| b.tag == "div" && b.content_rect.w > 700.0);
    assert!(b.is_some());
}
...
fn boxmodel_min_width() {
    ...
    assert!(b.is_some());
    assert!(b.unwrap().content_rect.w >= 200.0);
}
...
fn boxmodel_max_width() {
    ...
    assert!(b.is_some());
    assert!(b.unwrap().content_rect.w <= 305.0);
}
...
fn boxmodel_explicit_height() {
    ...
    let b = find_box(&doc.root, &|b| {
        b.tag == "div" && (b.content_rect.h - 100.0).abs() < 5.0
    });
    assert!(b.is_some());
}
...
fn boxmodel_display_none() {
    ...
    assert!(b.is_some());
    assert_eq!(b.unwrap().content_rect.w, 0.0);
    assert_eq!(b.unwrap().content_rect.h, 0.0);
}
