...
fn incremental_cascade_correctness() {
    ...
    mark_hover_dirty(&mut doc_inc.root, &old_chain, &hover_chain_inc, false, &HashSet::new());
...
fn incremental_cascade_skips_clean_subtrees() {
    ...
    mark_hover_dirty(&mut doc.root, &old_chain, &hover_chain, false, &HashSet::new());
    ...
    mark_hover_dirty(&mut doc.root, &hover_chain, &chain_nav1,
        doc.stylesheet.has_hover_descendant_rules, &HashSet::new());
