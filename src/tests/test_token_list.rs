...
fn value_is_verbatim_while_length_counts_the_parsed_set() {
    ...
    assert_eq!(cl.value(), "a a b  c", "Chrome: value=\"a a b  c\" — the attribute, untouched");
    assert_eq!(cl.length(), 3, "Chrome: length=3 — a, b, c");
    assert_eq!(cl.item(0).as_deref(), Some("a"));
    assert_eq!(cl.item(1).as_deref(), Some("b"), "the DUPLICATE a does not take slot 1");
...
fn add_and_remove_take_several_tokens_and_rewrite_the_serialized_set() {
    ...
    assert_eq!(d.get_attribute(el, "class").as_deref(), Some("a b c x y"),
...
fn toggle_reports_presence_after_and_force_pins_the_direction() {
    ...
    assert_eq!(d.class_list_mut(el).toggle("b", None), Ok(false), "Chrome: toggle b -> false");
...
fn replace_swaps_in_place_and_answers_false_for_a_token_that_is_not_there() {
    ...
    assert_eq!(d.get_attribute(el, "class").as_deref(), Some("q y"),
        "Chrome: replace x->q true class=\"q y\" — position kept");

    assert_eq!(d.class_list_mut(el).replace("nope", "q"), Ok(false));
    assert_eq!(d.get_attribute(el, "class").as_deref(), Some("q y"), "and nothing was written");
...
fn a_bad_token_is_rejected_before_anything_is_written() {
    ...
    assert_eq!(d.class_list_mut(el).add(&[""]), Err(TokenError::Syntax),
        "Chrome: add('') throws SyntaxError");
    assert_eq!(d.class_list_mut(el).add(&["a b"]), Err(TokenError::InvalidCharacter),
        "Chrome: add('a b') throws InvalidCharacterError");

    // The good token in front of the bad one must not have landed.
    assert_eq!(d.class_list_mut(el).add(&["good", "bad token"]), Err(TokenError::InvalidCharacter));
    assert_eq!(d.get_attribute(el, "class").as_deref(), Some("a"),
...
fn supports_answers_for_rel_and_is_a_type_error_for_class() {
    ...
    assert_eq!(d.class_list(link).supports("a"), None,
...
fn setting_value_writes_the_attribute_without_normalising() {
    ...
    assert_eq!(d.get_attribute(el, "class").as_deref(), Some("  p   q "),
...
fn an_element_with_no_such_attribute_does_not_grow_one() {
    ...
    assert_eq!(d.class_list(el).value(), "", "Chrome: value=\"\" with no attribute");
