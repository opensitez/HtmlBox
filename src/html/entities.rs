//! WHATWG HTML §13.5 — the named character reference table.
//!
//! Generated from the spec's own table (`data/ecma/whatwg-html/spec.html`,
//! "named character references"). It carried 62 hand-written entries, so
//! `&Aacute;` `&boxdl;` `&nsub;` `&sqcup;` and two thousand others reached the
//! DOM as literal text.
//!
//! Keys keep the trailing `;` where the spec has one, because the spec's
//! matching step consumes the semicolon as PART of the name: `&amp` and
//! `&amp;` are two separate rows, and only the ones without a semicolon are
//! the legacy forms that a text run may still resolve.

/// Longest key in the table, in bytes — the lookahead window.
pub const MAX_NAME_LEN: usize = 32;

/// `(name, expansion)`, sorted by name so a binary search can find it.
pub static NAMED_REFS: [(&str, &str); 2231] = [
    ("AElig", "\u{c6}"),
    ("AElig;", "\u{c6}"),
    ("AMP", "&"),
    ("AMP;", "&"),
...
/// Look up a name (semicolon included when the source had one).
pub fn lookup(name: &str) -> Option<&'static str> {
    NAMED_REFS
        .binary_search_by(|(k, _)| (*k).cmp(name))
        .ok()
        .map(|i| NAMED_REFS[i].1)
}
